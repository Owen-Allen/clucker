...
export default function ProfileHeader({ user, profile, menu, setMenu }: any) {
    ...
    const [loadedIsFollowing, setLoadedIsFollowing] = useState(false)
    const [followerCount, setFollowerCount] = useState(profile.followers.length) // allows us to increase or decrease the count if they click follow / unfollow button
    ...
    useEffect(() => {
        ...
        const isFollowingThisProfile = async () => {
            const response = await fetch(`${process.env.DB_HOST}/api/follow_detail/?user_id=${user.id}&following=${profile.id}`, { cache: 'no-store' })
            if (response.status === 200) {
                setIsFollowing(true)
            } else {
                setIsFollowing(false)
            }
            setLoadedIsFollowing(true)
        }
        if (!isMyProfile) {
    ...
    return (
        <Card className="border-black rounded-2xl border-2 border-b-4 border-r-4 shadow-xl">
            <div className="flex flex-row">
                <CardHeader className='w-3/5 sm:w-3/4'>
                    <CardTitle>
                        <Link className="text-lg hover:underline break-words" href={`/user/${profile.id}`}>
                            {profile.id}
                ...
                <CardHeader className="ml-auto">
                    ...
                    {isMyProfile ? 
                    <Link href={`/user/${user.id}/?editProfile=true`}>
                    ...
                    : 
                    loadedIsFollowing && <FollowButton isFollowing={isFollowing} followHandler={followHandler} />}
                </CardHeader>
            </div>
            ...
            <div className="flex flex-row">
                ...
                <div className='ml-auto text-xs pr-4 flex flex-col sm:flex-none'>
                    <Link className='hover:underline px-4' href={`/user/${profile.id}/followers`}>{followerCount} followers</Link> 
                    <Link className='hover:underline px-4' href={`/user/${profile.id}/following`}>{profile.following.length} following</Link>
                </div>
            </div>
            ...
            <CardContent className='pt-4'>
                <CardDescription className="text-sm break-words">
                    {profile.about}
                    {/* <p></p> */}
                </CardDescription>
            </CardContent>
